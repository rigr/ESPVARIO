...
// ---- watchdog ----
#define WDT_TIMEOUT_SECONDS 8   // 8 Sekunden – 
...
int updateCounter = 0;
const int FULL_REFRESH_EVERY = 40;  // Alle 40 Messungen statt 10
...
const unsigned long displayUpdateInterval = 500;  // Max alle 3 Sekunden
...
void setup() {
  ...
  // ── Echter Task Watchdog ───────────────────────────────────────────────
  Serial.println("Initialisiere Task Watchdog Timer (" + String(WDT_TIMEOUT_SECONDS) + "s)...");

  esp_task_wdt_deinit();  // Sicherheit: vorherigen WDT (falls vorhanden) abschalten

  esp_task_wdt_config_t twdt_config = {
    .timeout_ms       = WDT_TIMEOUT_SECONDS * 1000,   // in Millisekunden
    .idle_core_mask   = (1 << portNUM_PROCESSORS) - 1, // beide Kerne überwachen (bei S3 meist 2 Kerne)
    .trigger_panic    = true                           // bei Timeout Panic + Reset auslösen
  };

  esp_err_t err = esp_task_wdt_init(&twdt_config);
  if (err == ESP_OK) {
    Serial.println("Task Watchdog erfolgreich initialisiert");
  } else {
    Serial.printf("Task Watchdog Init fehlgeschlagen! Error: %d\n", err);
  }

  // Aktuellen Task (loop()-Task) zum Watchdog hinzufügen
  err = esp_task_wdt_add(NULL);  // NULL = aktueller Task
  if (err == ESP_OK) {
    Serial.println("loop()-Task zum Watchdog hinzugefügt");
  } else {
    Serial.printf("Konnte Task nicht zum Watchdog adden! Error: %d\n", err);
  }
  // ────────────────────────────────────────────────────────────────────────
  ...
  printMemoryInfo();
  esp_task_wdt_reset();  // ← gleich nach potenziell langsamem I2C
...
void loop() {
  ...
  esp_task_wdt_reset();   // ← Das ist das wichtigste! Mindestens alle 4–6 Sekunden rufen
